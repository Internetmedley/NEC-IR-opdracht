...
/// \details
/// This member function takes a decoded message and checks whether the last 5 bits of the message
/// equals an XOR of bits 2 through 11. Returns true if the message is a valid one.
...
/// \brief
/// Send decoded message member function
/// \details
/// This member function sends a decoded message and decides which listener to send it to.
/// If the player number in the received message is player 0, the message is a command instead of a hit and
/// the message will be sent to the object that listens to the commands. Hits will be sent to the hit listener.
void NEC::msg_decoder::send_message( uint16_t msg ) {
    if( (msg & 0x3E) == 0 ) {                               	//als speler nr een 0 is, is het een commando, anders een hit
        cmd_l.msg_received( (msg & 0x7C0) >> 6 );            	//stuurt alleen data section en niet de start bit eruit geshift
    }
    else {
        hit_l.msg_received( (msg & 0x7FE) >> 1 );            	//dit stuurt alleen data en speler nummer en start bit eruit geshift
    }
